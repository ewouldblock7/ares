...
void CacheTest::TestCacheTable() {

    {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            ss << "key_" << i;
            std::string key = ss.str();
            CacheHandle * handle = table.Get(key,
                    Hash(key.c_str(), key.size()));
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            ss << "key_" << i;
            std::string key = ss.str();
            CacheHandle * handle = table.Remove(key,
                    Hash(key.c_str(), key.size()));
            EXPECT_EQ(handle, handles[i]);
            if (handle != handles[i]) {
...

    {
        ...
        for (uint32_t i = 0; i < count; ++i) {
            ...
            handle = table.Get(key_noexist,
                    Hash(key_noexist.c_str(), key_noexist.size()));
            EXPECT_TRUE(handle == NULL);
            handle = table.Remove(key_noexist,
                    Hash(key_noexist.c_str(), key_noexist.size()));
            EXPECT_TRUE(handle == NULL);
...
void CacheTest::TestStringCache() {
    ...
    {
        boost::shared_ptr<StringCache> strcache_ptr = newShardedLRUStringCache(
                10000, 1024 * 1024 * 10);
        uint32_t count = 1024;
        for (uint32_t i = 0; i < count; ++i) {
            std::stringstream ss;
