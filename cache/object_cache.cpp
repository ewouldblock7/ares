...
public:
    LRUObjectCache(uint64_t cap_charge, uint32_t cap_count, uint32_t idx = 0) :
            cap_charge_(cap_charge), cap_count_(cap_count), charge_(0), count_(
                    0), idx_(idx) {
    ...
    void Insert(const Slice & key, boost::shared_ptr<void> & value,
        ...
        ++count_;
        charge_ += charge;
        while ((count_ > cap_count_ || charge_ > cap_charge_)
                && head_.lru_prev_ != &head_) {
...
public:
    LRUShardedObjectCache(uint64_t capacity, uint32_t max_elenum) :
            capacity_(capacity), max_elenum_(max_elenum), shards_(NULL), shard_num_(
                    LRU_SHARD_NUM), shard_num_bits_(LRU_SHARD_BITS) {
        inital();
    }
...
private:
    ...
    void inital() {
        ...
        for (uint32_t i = 0; i < shard_num_; ++i) {
            shards_[i] = new LRUObjectCache(capacity_ / shard_num_,
                    max_elenum_ / shard_num_, i);
        }
    }
...
extern boost::shared_ptr<ObjectCache> newShardedLRUObjectCache(
    ...
    return boost::shared_ptr<ObjectCache>(
            new LRUShardedObjectCache(charge_cap, count_cap));
}
