...
public:
    CacheTable() :
            slots_(NULL), slots_2_(NULL), slot_size_2_(0), rehashing_idx_(0), slot_size_(
                    0), ele_num_(0) {
        Resize2();
    }
...
private:
    ...
    T ** FindPtr(T ** slots, uint32_t slot_size, const Slice & key,
            uint32_t hash) {
        T ** ptr = &slots[hash & (slot_size - 1)];
        while (*ptr != NULL && ((*ptr)->hash_ != hash || (*ptr)->key() != key)) {
            ptr = &((*ptr)->next_);
        }
        return ptr;
    }
